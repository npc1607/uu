...
func (i *App) configRouter() error {
	...
	case router.SteamDeck:
		return i.checkSteamDeckIPTables()
	default:
		return fmt.Errorf("unsupported router %q", i.params.router)
	}
}

func (i *App) checkSteamDeckIPTables() error {
	if _, err := exec.LookPath("iptables"); err != nil {
		return fmt.Errorf("iptables is required by the current UU plugin: %w", err)
	}
	output, err := i.commandOutput("iptables", "--version")
	if err != nil {
		return fmt.Errorf("iptables is not usable: %s; if the kernel was recently upgraded, reboot first", strings.TrimSpace(output))
	}
	return nil
}
